...
public class Main {
	public static void main(String[] args) {
		...
		do {
			...
			case 4:	// 수정 (검색어를 입력받아서 검색어를 hd에게 전달, 수정여부를 결과로 받아서 출력)
				System.out.print("검색할 사람의 이름 입력 : "); name = sc.nextLine();
				System.out.print("수정할 사람의 나이 입력 : "); age = Integer.parseInt(sc.nextLine());
				ob = new Human(name, age);
				row = hd.update(ob);
				// ob.name과 이름이 일치하는 객체를 찾아서, 그 객체의 나이를 ob.age로 덮어씌우기
				if(row == 1) System.out.println("수정 완료 되었습니다");
				if(row == 0) System.out.println("결과가 없습니다");
				break;
				
			case 5:	// 삭제 (검색어를 입력받아서 검색어를 hd에게 전달, 삭제여부를 결과로 받아서 출력)
